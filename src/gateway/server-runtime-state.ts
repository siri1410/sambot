...
export async function createGatewayRuntimeState(params: {
  ...
  // Resolve Google OAuth config and create handlers
  const googleOAuthConfig = resolveGoogleOAuthConfig({
    oauthConfig: params.cfg.googleOAuth,
    env: process.env,
  });
  const oauthHandlers = googleOAuthConfig
    ? createGoogleOAuthHandlers(googleOAuthConfig)
    : createNoopOAuthHandlers();
  ...
  for (const host of bindHosts) {
    const httpServer = createGatewayHttpServer({
      ...
      handleOAuthRequest: oauthHandlers.handleOAuthRequest,
      oauthMiddleware: googleOAuthConfig ? oauthHandlers.authMiddleware : undefined,
    });
    try {
