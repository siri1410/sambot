...
export function resolveGatewayPort(
  ...
  // Check CLAWDBOT_GATEWAY_PORT first (explicit override)
  const clawdbotPort = env.CLAWDBOT_GATEWAY_PORT?.trim();
  if (clawdbotPort) {
    const parsed = Number.parseInt(clawdbotPort, 10);
  ...
  // Check PORT (standard PaaS variable, e.g., Railway, Heroku, Render)
